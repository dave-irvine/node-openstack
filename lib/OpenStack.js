...
(function() {
  ...
  module.exports.OpenStack = (function(_super) {
    ...
    OpenStack.prototype.auth = function(fn) {
      ...
      return this.base_post(urljoin(this.options.endpoints.identity, "/auth/tokens"), authOpts, (function(body, headers) {
        debug("auth complete");
        this.auth_token = {
    ...
    OpenStack.prototype.switchContext = function(context, fn) {
      ...
      return this.post(urljoin(this.options.endpoints.identity, "/auth/tokens"), authOpts, (function(body, headers) {
    ...
    OpenStack.prototype.get = function(path, query, fn) {
      ...
      return this.checkAuth((function() {
        var _get;
        _get = (function() {
          this.options.request_headers = _.extend({
            "X-Auth-Token": this.auth_token.id
          }, this.options.default_headers);
          path = this.fixPath(path);
          path = this.replaceTokens(path, query);
          delete query.context;
          return OpenStack.__super__.get.call(this, path, query, fn);
        }).bind(this);
        if (query.context && query.context !== "%context%" && query.context !== this.auth_token.context) {
          debug("switch context to " + query.context);
          return this.switchContext(query.context, _get);
        } else {
          return _get();
        }
      }).bind(this));
    ...
    OpenStack.prototype.post = function(path, query, fn) {
      ...
      return this.checkAuth((function() {
        ...
        _post = (function() {
          ...
          path = this.replaceTokens(path, query);
          delete query.context;
          return OpenStack.__super__.post.call(this, path, query, fn);
        }).bind(this);
        if (query.context && query.context !== "%context%" && query.context !== this.auth_token.context) {
          debug("switch context to " + query.context);
          return this.switchContext(query.context, _post);
        } else {
          return _post();
        }
      }).bind(this));
